...
| `agents/knowledge.py` | Local LanceDB index over the user's corpus + seed URLs. **Hybrid retrieval** — vector + full-text (BM25), fused by RRF (ADR 0011) — plus the tiered `is_weak` relevance check, source-tier classification (ADR 0009), and `<sources>` formatting. | `list[Passage]` | embeddings only (bge-small via fastembed/ONNX) |
